...
class NICE_SLAM:
    ...
    def load_bound(self, cfg):
        ...
        if self.nice:
            ...
            if self.coarse:
                self.shared_decoders.coarse_decoder.bound = (
                    self.bound * self.coarse_bound_enlarge
                )
    ...
    def load_pretrain(self, cfg):
        ...
        if self.coarse:
            ckpt = torch.load(
                cfg["pretrained_decoders"]["coarse"],
        ...
        ckpt = torch.load(
            cfg["pretrained_decoders"]["middle_fine"],
            map_location=cfg["mapping"]["device"],
        )
        middle_dict = {}
        fine_dict = {}
    ...
    def grid_init(self, cfg):
        ...
        if self.coarse:
            coarse_val_shape = list(
                map(
                    int,
                    (xyz_len * self.coarse_bound_enlarge / coarse_grid_len).tolist(),
                )
            )
            coarse_val_shape[0], coarse_val_shape[2] = (
                coarse_val_shape[2],
                coarse_val_shape[0],
            )
