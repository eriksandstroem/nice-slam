...
class Mapper(object):
    ...
    def optimize_map(
        ...
        for joint_iter in range(num_joint_iters):
            ...
            if (not (idx == 0 and self.no_vis_on_first_frame)) and (
                "Demo" not in self.output
            ):
                self.visualizer.vis(
                    idx,
                    joint_iter,
        ...
        while 1:
            while True:
                ...
                if self.sync_method == "strict":
                    ...
                    # the mapping process only runs when the tracking idx is at least a bit ahead.
