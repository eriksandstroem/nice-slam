...
class Tracker(object):
    ...
    def optimize_cam_in_batch(
        self, camera_tensor, gt_color, gt_depth, batch_size, optimizer
    ):
        """
        ...
        loss = (torch.abs(batch_gt_depth - depth) / torch.sqrt(uncertainty + 1e-10))[
            mask
        ].sum()
    ...
    def run(self):
        ...
        for idx, gt_color, gt_depth, gt_c2w in pbar:
            ...
            if self.sync_method == "strict":
                ...
                # mapping idx can be later than tracking. The mapping idx is within the bounds of [-self.every_frame-self.every_frame//2, -self.every_frame+self.every_frame//2] compared to the tracking idx.
                while (
                    # tracking process waits if it runs too far away from mapping process
                    self.mapping_idx[0]
                    < idx - self.every_frame - self.every_frame // 2
                ):
                    time.sleep(0.1)
            ...
            if idx == 0 or self.gt_camera:
                ...
                if self.const_speed_assumption and idx - 2 >= 0:
                    ...
                    delta = (
                        pre_c2w
                        @ self.estimate_c2w_list[idx - 2].to(device).float().inverse()
                    )
